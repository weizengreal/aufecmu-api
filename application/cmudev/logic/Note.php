 ...
 * Time: 上午11:38
 */
namespace app\cmudev\logic;

use think\Db;
use think\Log;
use memcacheLoad\FindCache;

use \app\cmudev\model;

class Note  {

    public function __construct()
    {
        $this->FindCache = new FindCache('dev');
    }
    ...
    public function initModel() {
        ...
        $this->note = new model\Note();
    }

    /*
     * 初始化数据库中的数据到memcache缓存中
     * */
    public function initMem() {
        $this->initModel();
        $originData = $this->note->getLimitData(0,MEMCACHE_COUNT);
        $themeFormat = $this->note->getThemeCount(MEMCACHE_COUNT);
        $this->FindCache->initThemeData($originData,$themeFormat);
    }
     ...
     * 获取数据
     * */
    public function _getAllData($page,$type = null) {
        $res = $this->FindCache->getThemeData($page,empty($type) ? 'find' : $type); //$fetch
        if( !empty($res['errcode']) && $res['errcode'] == 3002 ) {
            $note = new \app\cmudev\model\Note();
            $res = $note->getLimitData($page,ONE_PAGE_COUNT,$type,true);
        }
    ...
    public function test() {
        $mem = new \think\cache\driver\Memcached();
        return $mem->get('memcache_api_product_find');
    }
...
//            $res[$index]['testid']=authcode($res[$index]['noteid'],"DECODE");
     ...
     * 实现获取数据
     * */
    public function getData($type, $start,$count)
    {
        if( empty($type) || $type == 'find' ) {
            $whereStr =  "club_note.display=1";
        }
    ...
    public static function newComment($postData) {
        ...
        if(!empty($backUserInfo)) {
            ...
            $comId = Db::table("club_comment")->insertGetId($comArr);
            if($comId !== false &&
                Db::table("club_note")->where("`id`='{$postData['id']}'")->setInc("comcount",1) !== false) {
                // 发送模板消息
                $asyncMsg = new \swoole\AsyncMsg();
                $asyncMsg->connect($comId,$openid);
...
//                }
    ...
    public static function zan($postData) {
        $postData = array_merge($postData,['thing'=>1]);
